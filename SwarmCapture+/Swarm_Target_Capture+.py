...
_SIM_STATE_NAMES = ['Agents', 'Agents_Bodies', 'Agents_History', 'DockPose', 'IMU', 'Landm', 'Lc', 'Lcd', 'N', 'Random_State', 'Rant', 'Rcol', 'Rcom', 'Rdet', 'Rdet_ap', 'Rflk', 'Spacecraft', 'Target_History', 'Target_Point_Cloud_History', 'TexCub_id', 'X', 'a', 'age_rot', 'agent_body_id', 'agent_collision_shape_id', 'agent_inertia', 'agent_mass', 'agent_size', 'agent_state', 'agent_visual_shape_id', 'altitude', 'angvel', 'ap', 'attachment_points', 'avg_time_per_iteration', 'body_id', 'camera_distance', 'camera_pitch', 'camera_target_position', 'camera_yaw', 'cancel_chw', 'cfg', 'chw_force', 'cond_save', 'cond_viz', 'constraint', 'contact_point', 'control_force', 'data', 'delay', 'df', 'dis', 'dq', 'ds', 'dt', 'duration', 'elapsed_time', 'estimated_time_left', 'estimated_time_left_str', 'every', 'file', 'force', 'force_thrust', 'g0', 'iterations_left', 'k', 'kappa', 'lidar_fov', 'low_pass_filter_coeff_control', 'low_pass_filter_coeff_state', 'max_angle_pcd', 'max_distance', 'max_frc', 'max_lidar_pcd', 'max_trq', 'mode', 'mode_prev', 'neighbour_weight', 'ngh_action_time', 'num', 'num_feat', 'num_iter', 'num_rays_phi', 'num_rays_theta', 'obj_file', 'object_name', 'orientation', 'orientations', 'output_file', 'paths', 'physicsClient', 'point', 'pos', 'position', 'positions', 'q', 'quat', 'querried_agent', 'r', 'rand_ord', 'ray_results', 'rel_pos', 'rel_rot', 'sim_params', 'start_time', 'std', 'step', 'tag', 'tar', 'tar_angvel', 'tar_pos', 'tar_quat', 'tar_rot', 'tar_vel', 'target_body_id', 'target_kdtree', 'target_orientation', 'target_pcd', 'target_position', 'target_state', 'texTar_id', 'texture_file', 'thruster_Isp', 'torque', 'torque_thrust', 'u', 'vel', 'visualize_fraction', 'visualize_hits', 'visualize_rays', 'viz_agent_pcd', 'viz_target_pcd', 'voxel', 'writer']


def _restore_sim_globals(sim_state):
    globals().update(sim_state)


def _capture_sim_globals(sim_state):
    for name in _SIM_STATE_NAMES:
        if name in globals():
            sim_state[name] = globals()[name]


def _initialize_simulation_body(sim_state):
    global Agents, Agents_Bodies, Agents_History, IMU, N, Random_State, Rant, Rcol, Rcom, Rdet, Rdet_ap, Rflk
    global Target_History, Target_Point_Cloud_History, TexCub_id, X, a, agent_body_id, agent_collision_shape_id, agent_inertia, agent_mass, agent_size, agent_state, agent_visual_shape_id
    global altitude, angvel, ap, attachment_points, camera_distance, camera_pitch, camera_target_position, camera_yaw, cancel_chw, cfg, delay, ds
    global dt, duration, g0, lidar_fov, low_pass_filter_coeff_control, low_pass_filter_coeff_state, max_angle_pcd, max_distance, max_frc, max_lidar_pcd, max_trq, num_feat
    global num_iter, num_rays_phi, num_rays_theta, obj_file, object_name, orientations, paths, physicsClient, pos, positions, quat, sim_params
    global start_time, std, step, tag, tar_angvel, tar_vel, target_body_id, target_orientation, target_pcd, target_position, target_state, texTar_id
    global texture_file, thruster_Isp, vel, visualize_fraction, visualize_hits, visualize_rays, viz_agent_pcd, viz_target_pcd, voxel
...
def _step_simulation_body(sim_state):
    ...
    global ngh_action_time, num, orientation, output_file, point, pos, position, q, quat, querried_agent, r, rand_ord
    global ray_results, rel_pos, rel_rot, step, tar, tar_angvel, tar_pos, tar_quat, tar_rot, tar_vel, target_kdtree, target_state
    global torque, torque_thrust, u, vel, writer
    _restore_sim_globals(sim_state)
...
def initialize_simulation(config=None):
    ...
    sim_state = {
        "__runtime_cfg": asdict(runtime_config),
        "__resolved_paths": _resolve_simulation_paths(runtime_config),
    }
    _initialize_simulation_body(sim_state)
